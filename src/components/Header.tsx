    ...
    <header className={cn(
      ...
      <motion.div
        ...
        className={cn(
          ...
          isScrolled
            ? "max-w-6xl rounded-full border border-border/40 bg-background/80 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.12)] px-6"
            : "max-w-full bg-background border-b border-border/10 px-8"
        )}
      >
        <div className={cn(
            ...
            <div className="relative flex items-end">
              <img
                className={cn(
                  ...
                  isScrolled ? "w-24 mt-0" : "w-40 mt-4"
                )}
                src={logo}
                alt="Inowix Logo"
              />
              <span className={cn(
                ...
                isScrolled ? "text-sm mb-1.5 -ml-2" : "text-xl mb-4 -ml-3"
              )}>.in</span>
            </div>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-1">
            ...
            >
              <div className="p-2 w-56">
                ...
                <DropdownItem to="/schbang-network">Inowix Network</DropdownItem>
              </div>
            </NavDropdown>
          ...
          <div className="flex items-center gap-3">
            ...
            <Button asChild size="sm" className={cn(
              "hidden sm:flex rounded-full transition-all duration-500 font-semibold",
              isScrolled ? "h-9 px-5 bg-primary hover:bg-primary/90" : "h-11 px-7 bg-primary hover:bg-primary/90"
            )}>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="lg:hidden absolute top-full left-0 right-0 p-4 z-[110]"
          >
            <div className="bg-background/95 backdrop-blur-2xl border border-border/50 rounded-[2rem] p-8 shadow-2xl space-y-8 overflow-y-auto max-h-[85vh]">
              {/* Mobile Links */}
              ...
              <MobileSection title="About">
                ...
                <MobileSubLink to="/schbang-network" onClick={() => setIsMobileMenuOpen(false)}>Inowix Network</MobileSubLink>
              </MobileSection>
              ...
              <div className="pt-4 space-y-4">
                {user && isAdmin && (
                  <Button asChild variant="outline" className="w-full rounded-2xl py-6 border-primary text-primary">
                    <Link to="/admin" onClick={() => setIsMobileMenuOpen(false)}>Admin Panel</Link>
                  </Button>
                )}
                <Button asChild className="w-full rounded-2xl py-7 text-lg">
...
const MobileNavLink = ({ to, onClick, children }: any) => (
  <Link to={to} onClick={onClick} className="block text-3xl font-bold tracking-tight hover:text-primary transition-colors">
    {children}
  </Link>
);
...
const MobileSubLink = ({ to, onClick, children }: any) => (
  <Link to={to} onClick={onClick} className="text-xl font-medium text-foreground/80 hover:text-primary transition-colors">
    {children}
  </Link>
);